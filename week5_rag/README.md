...
本系统提供了一套完整的复杂检索测试案例，用于评估 RAG 系统在多跳推理、复杂上下文理解和抗幻觉能力方面的表现。
...
4. **逐个提问**
   根据 `answer.md` 中的测试问题逐个输入：
   - 多跳推理类问题：
     1. "谁负责AI助手开发项目？他的直接上级是谁？上级的入职时间是什么时候？"
     2. "市场部负责的项目预计什么时候上线？该项目的合作伙伴有哪些？"
     3. "公司在获得A轮融资后多久推出了第一款AI产品？"
     4. "如果项目Gamma需要与上海分公司协作，应该联系哪个部门？"
     5. "研发部和市场部合作的项目有哪些？这些项目的预计上线时间是什么时候？"
   - 幻觉测试类问题：
     6. "公司的物流部门负责人是谁？"
     7. "刘洋负责的项目预计什么时候上线？"
     8. "2020年加入公司的员工负责哪个项目？"
     9. "哪些员工不是2021年之后加入公司的？"
     10. "哪个部门的负责人加入公司时间最长？"

5. **记录结果**
   - 将每个问题的实际回答记录到 `result.md` 文件中
   - 对比正确答案，分析系统的表现
...
| 文件 | 用途 |
|------|------|
| `test_multihop.md` | 测试文档，包含公司组织结构、项目信息、人员信息等 |
| `answer.md` | 测试问题及正确答案参考 |
| `result.md` | 测试结果记录文件（需手动填写） |

**测试结果分析：**

完成测试后，可以分析以下方面：
- **多跳推理能力**：系统能否正确整合多个来源的信息
- **抗幻觉能力**：系统是否会编造不存在的信息
- **分块影响**：分块策略对检索结果的影响
- **检索准确性**：检索结果是否相关且完整

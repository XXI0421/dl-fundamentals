...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, kv_cache=None, layer_idx=0, freqs_cis=None): # 同7
        B, T, C = x.size()
        ...
        if freqs_cis is not None:
            # 注意：freqs_cis 是 (T, d_k//2)，需要适配多头
            # 扩展维度以匹配 (B, H, T, d_k//2) 的广播
...
class TransformerBlock(nn.Module):
    ...
    def forward(self, x, kv_cache=None, layer_idx=0, freqs_cis=None): # 修改7: 传递给attention
        attn_out, _ = self.attn(x, kv_cache=kv_cache, layer_idx=layer_idx,  freqs_cis=freqs_cis) # 同7
        x = self.norm1(x + self.dropout(attn_out))
        x = self.norm2(x + self.ff(x))
        return x

class CharTransformer(nn.Module):
    def __init__(self, max_seq_len=512): # 修改1:增加最大长度支持
        super().__init__()
        self.token_embed = nn.Embedding(vocab_size, d_model)
        # 修改2: RoPE不需要可学习的位置嵌入
        # self.pos_embed = nn.Embedding(seq_len, d_model)
        
        # 修改3: 预计算 RoPE 频率（支持到 max_seq_len）
        self.register_buffer("freqs_cis",
            precompute_freqs_cis(d_model // num_heads, max_seq_len))
    ...
    def forward(self, x, targets=None, kv_cache=None):
        ...
        # 修改5: 截取当前序列长度的频率
        # freqs_cis 形状是 (max_seq_len, dim//2)，取前 T 个
        freqs_cis = self.freqs_cis[:T]

        # 修改6: 传递给所有 block
        for layer_idx, block in enumerate(self.blocks):
            x = block(x, kv_cache=kv_cache, layer_idx=layer_idx, freqs_cis=freqs_cis)
